...
function unicodeToFont(font, text) {		
	...
	// Loop that iterate through all available symbol
	for(var i in fontRespectiveUnicode[font]['symbol']) { 
		let __fontUnicode = fontRespectiveUnicode[font]['symbol'][i];	
	...
	for(var i in fontRespectiveUnicode[font]['font-keymap']) { 

		let __fontUnicode = fontRespectiveUnicode[font]['font-keymap'][i];	
		// If letter has multiple variant in the font then else part will executes 
		if (typeof __fontUnicode == "string") { 
			i = convertMixedUnicodeToText(i);
        	let searchChar = htmlDecode(i);
